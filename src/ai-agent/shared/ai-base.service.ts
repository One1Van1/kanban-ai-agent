...
// import { CheckEntityExistsService } from '../check-entity-exists/check-entity-exists.service'; // НЕ СТРИЖКИ: закомментировано
import {
  TaskAnalysisResult,
  TaskProgressResult,
...
export class AiBaseService {
  ...
  constructor(
    ...
    // protected readonly checkEntityExistsService: CheckEntityExistsService, // НЕ СТРИЖКИ: закомментировано
  ) {}

  /**
   * Анализ задачи из колонки New (только для стрижек)
   * НЕ СТРИЖКИ: общая логика и логика сущностей закомментированы
   */
  protected analyzeNewTask(
    ...
    // НЕ СТРИЖКИ: закомментировано - проверка на создание сущности
    ...
    // Проверяем, является ли задача о стрижке/красоте
    if (this.isHaircutRelated(text)) {
      return this.analyzeHaircutTask(taskKey, summary, description);
    }

    // НЕ СТРИЖКИ: закомментировано - общая логика для других задач
    // const clearKeywords = [
    //   'fix',
    //   'bug',
    //   'implement',
    ...
    // Если задача не о стрижках - оставляем в New
    return {
      taskKey,
      isUnderstandable: false,
      decision: 'stay_in_new',
      reason: 'Task is not haircut-related, keeping in New for manual review',
    };
  }
   ...
   */
  protected isHaircutRelated(text: string): boolean {
  ...
  /**
   * Специальный анализ задач о стрижке
   * Логика соответствует специализированному агенту analyze-haircut-tasks
   */
  private analyzeHaircutTask(
    taskKey: string,
    summary: string,
    description?: string,
  ): TaskAnalysisResult {
    this.logger.log(`Analyzing haircut task: ${taskKey}`);

    // Анализируем полноту информации о стрижке
    const hasTitle = Boolean(summary);
    const hasDescription = Boolean(description && description.trim());

    // TODO: Получить информацию о вложениях из Jira API
    // Пока упрощённая проверка на наличие ссылок на изображения в тексте
    ...
    if (hasTitle && hasDescription && hasPhotoReference) {
      return {
        ...
        isUnderstandable: true,
        decision: 'move_to_progress',
        reason: 'Haircut task has title, description and photo reference',
        // Комментарий добавляет специализированный сервис
      };
    }
    ...
    return {
      ...
      isUnderstandable: false,
      decision: 'move_to_questions',
      reason: 'Haircut task missing description and/or photo reference',
      // Комментарий добавляет специализированный сервис
    };
  }

  // НЕ СТРИЖКИ: закомментировано - весь блок методов для работы с сущностями
  // /**
  ...
  /**
   * Анализ задачи из колонки In Progress (только для стрижек)
   * НЕ СТРИЖКИ: логика для сущностей закомментирована
   */
  protected async analyzeProgressTask(
    ...
    // Проверяем, является ли задача о стрижках
    if (this.isHaircutRelated(text)) {
      // Для стрижек - всегда готовы к review после выполнения
      return {
        taskKey,
        isCompleted: true,
    ...
    // Если задача не о стрижках - оставляем в progress для ручного управления
    return {
      taskKey,
      isCompleted: false,
      decision: 'stay_in_progress',
      reason: 'Non-haircut task, keeping in progress for manual review',
    };
  }

  // НЕ СТРИЖКИ: закомментировано - все методы для работы с сущностями
  // /**
