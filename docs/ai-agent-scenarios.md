...
### 6. **Анализ задач о стрижках** (`analyze-haircut-tasks`)

**Эндпоинт:** `POST /ai-agent/analyze-haircut-tasks`

**Описание:** AI анализирует задачи в колонке "New", связанные с парикмахерскими услугами, и автоматически сортирует их по полноте информации.

**Логика работы:**

- **Полные задачи** (название + описание + фото) → перемещаются в колонку **"In Progress"**
- **Неполные задачи** (только название) → перемещаются в колонку **"Questions"** + добавляется комментарий "Какую именно стрижку ты хочешь?"

**Критерии анализа:**

- Проверяет наличие ключевых слов: "стрижк", "haircut", "причёск", "парикмахер", "hair", "волос", "укладк", "стиль", "сделай мне", "подстриг"
...
**Полная задача (→ In Progress):**

```
Название: Сделай мне такую стрижку
Описание: Хочу стрижку как на этой фотографии
Вложения: [фото стрижки]
```

**Неполная задача (→ Questions):**
...
**Описание:** AI анализирует задачи в колонке "New", связанные с парикмахерскими услугами (стрижками), и автоматически сортирует их по полноте информации.

**Логика работы:**

- **Полные задачи** (название + описание + фото) → перемещаются в колонку **"In Progress"**
- **Неполные задачи** (только название) → перемещаются в колонку **"Questions"** с комментарием "Какую именно стрижку ты хочешь?"

**Критерии анализа:**

- Проверяет наличие ключевых слов: "стрижк", "haircut", "причёск", "парикмахер", "волос", "сделай мне"
- Анализирует полноту информации (название, описание, вложения)
- Автоматически перемещает задачи в соответствующие колонки
...
### Примеры задач о стрижках:

#### ✅ **Полные задачи** (→ In Progress):

- "Сделай мне такую стрижку" + описание + фото
- "Хочу стрижку боб" + детальное описание + референсное фото
- "Подстриги волосы как на картинке" + описание + вложение

#### ❓ **Неполные задачи** (→ Questions + комментарий):

- "Сделай мне такую стрижку" (без описания и фото)
- "Подстриги меня" (без деталей)
- "Хочу новую прическу" (без конкретики)
