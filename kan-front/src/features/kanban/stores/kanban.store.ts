...
export const useKanbanStore = create<KanbanStore>((set, get) => ({
  ...
  fetchBoards: async () => {
    ...
    try {
      ...
      // const response = await kanbanAPI.boards.list();
  ...
  fetchBoard: async (boardId) => {
    set({ isLoading: true, error: null });
    try {
      const response = (await kanbanAPI.boards.getStructure(boardId)) as any;
      set({
  ...
  fetchTasksByColumn: async (columnName) => {
    set({ isLoading: true, error: null });
    try {
      const response = (await kanbanAPI.tasks.getByColumn(columnName)) as any;
  ...
  fetchTask: async (taskId) => {
    try {
      const response = (await kanbanAPI.tasks.getById(taskId)) as any;
  ...
  createTask: async (data) => {
    set({ isLoading: true, error: null });
    try {
      const response = (await kanbanAPI.tasks.create(data)) as any;
  ...
  updateTask: async (taskId, data) => {
    set({ error: null });
    try {
      const response = (await kanbanAPI.tasks.update(taskId, data)) as any;
  ...
  deleteTask: async (taskId) => {
    set({ error: null });
    try {
      await kanbanAPI.tasks.delete(taskId);
  ...
  moveTask: async (taskId, targetColumnId, position) => {
    set({ error: null });
    try {
      await kanbanAPI.tasks.move(taskId, {
  ...
  assignTask: async (taskId, assigneeEmail) => {
    set({ error: null });
    try {
      await kanbanAPI.tasks.assign(taskId, { assignee: assigneeEmail });
  ...
  fetchComments: async (taskId) => {
    try {
      const response = (await kanbanAPI.comments.getByTask(taskId)) as any;
  ...
  addComment: async (taskId, content) => {
    try {
      const response = (await kanbanAPI.comments.add(taskId, {
        text: content,
  ...
  updateComment: async (commentId, content) => {
    try {
      await kanbanAPI.comments.update(commentId, { text: content });
  ...
  deleteComment: async (commentId) => {
    try {
      await kanbanAPI.comments.delete(commentId);
