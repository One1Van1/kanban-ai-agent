...
export const useFlowBuilderStore = create<FlowBuilderStore>((set, get) => ({
  ...
  saveFlow: async (flow) => {
    ...
    try {
      ...
      if (isNewFlow) {
        // Create new flow
        savedFlow = await flowsAPI.create({
          name: flow.name,
        ...
        savedFlow = await flowsAPI.update(flow.id, {
          name: flow.name,
          description: flow.description,
          definition: {
            blocks: flow.blocks,
  ...
  loadFlows: async (params?: {
    ...
    try {
      ...
      // Convert backend response to frontend format
      const frontendFlows: FlowDefinition[] = response.items.map(
        (item: any) => ({
          id: item.flowId,
          name: item.name,
  ...
  deployFlow: async (flowId) => {
    ...
    try {
      const response = await flowsAPI.deployToAgent(flowId, {
